...
class MLP(nn.Module):
    def __init__(self, input_size, output_size):
        ...
        self.fc1 = nn.Linear(input_size, 256, dtype=torch.float32)
        self.fc2 = nn.Linear(256, 512, dtype=torch.float32)
        self.fc3 = nn.Linear(512, 40 * 40 * 320, dtype=torch.float32)  # Output size: 128x64x320
...
class ControlNet(nn.Module):
    def __init__(
        ...
        self.input_hint_block = TimestepEmbedSequential(
            nn.Linear(1, 256, dtype=torch.float32),
            nn.ReLU(),
            nn.Linear(256, 512, dtype=torch.float32),
            nn.ReLU(),
            zero_module(nn.Linear(512, 256 * 40 * 40, dtype=torch.float32))
            
            
        )
        
        self.mlp = MLP(input_size=1*1, output_size=320*40*40)
